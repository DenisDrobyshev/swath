...
векторизует маску в полигоны WGS84 с площадями. В `--input` можно передать и
каталог.
...
| `POST` | `/api/segment` | multipart-загрузка; возвращает маску, наложение, покрытие и ссылки |
...
Образ под CPU; отдаёт те чекпоинты, что примонтированы в `/models`.
